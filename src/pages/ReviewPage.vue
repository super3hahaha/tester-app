...
// dayTick 是响应式跨天信号：computed 内的 new Date() 不是响应式依赖，若不显式依赖
...
// 跨天检查：既要刷新 min/max（dayTick），也要把选中区间随之平移。可见性切回时调用，
// 另有定时器每分钟兜底（app 一直在前台不切走时 visibilitychange 不会触发）。
function checkDayRollover() {
  const today = todayIso();
  if (maxSelectableDate.value === today) return; // 未跨天，min/max 与区间都无需动
  // 先让 min/max computed 重算，解除「今天」被日期选择器置灰的问题
  dayTick.value++;
  // 再把整个区间向前平移相同天数，保持区间长度不变
  const oldTo = new Date(toDate.value).getTime();
  const oldFrom = new Date(fromDate.value).getTime();
  const spanDays = Math.round((oldTo - oldFrom) / 86400000);
  toDate.value = today;
  fromDate.value = daysAgoIso(Number.isFinite(spanDays) ? spanDays : 7);
}
function onVisibilityChange() {
  if (document.visibilityState !== "visible") return;
  checkDayRollover();
}
let dayRolloverTimer: ReturnType<typeof setInterval> | null = null;

onMounted(async () => {
  document.addEventListener("visibilitychange", onVisibilityChange);
  // 前台常驻时 visibilitychange 不会触发，每分钟兜底检查一次跨天
  dayRolloverTimer = setInterval(checkDayRollover, 60000);
