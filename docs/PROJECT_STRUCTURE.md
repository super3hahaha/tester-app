...
│   │   ├── reviewFavorites.ts   # 评论「收藏」：存整条评论快照（非仅 id，超出 7 天 API 窗口也能回看），键 review_id，`review-fav-v1`，经 scopedKey 按账号隔离；ReviewPage 写（卡片 ☆/★）、FavoriteReviewsPage 读 + 删；回复提交后 updateFavoriteReply() 顺带同步收藏夹里的回复状态
│   │   ├── mailFavorites.ts     # 邮件「收藏」：存整封邮件快照 + 收藏时的邮件源信息（_sourceKey 分组 / _sourceLabel+_sourceTab 拼「邮箱 · 分页」显示名，与邮件表下拉框一致 / _profileDir），键 messageId（缺失退回 link），`mail-fav-v1`，**全局不按账号隔离**（邮件源 gmail-sources-v1 本身就是全局的，scope 了切账号收藏夹会变空）；GmailPage 写（卡片/详情 ☆★）、FavoriteMailsPage 读 + 删
...
│       ├── FavoriteReviewsPage.vue # 收藏评论：挂在 Review 工作区（`review-favorites`）。列出 reviewFavorites.ts 里存的收藏评论快照（按收藏时间倒序），卡片复用 ReviewPage 同款展示（星级/正文/原文/meta/开发者回复），右下角 ★ 点击取消收藏；「在网页中打开」只读 `review-page-config-v3`（ReviewPage 维护的 developerId/appIdByPkg）拼深链，缺失则不显示按钮。不提供 AI 回复/模板回复/分析——纯回看 + 取消收藏
...
│       ├── FavoriteMailsPage.vue # 收藏邮件：挂在邮件工作区（`gmail-favorites`）。列出 mailFavorites.ts 里的邮件快照（按收藏时间倒序），按邮件源分 tab，列表卡片与 Gmail 页同款紧凑三行（发件人+日期+📎+收藏时间 / 主题 / 机翻中文，均单行截断），「详情」弹同款大卡（机翻上/原文下），★ 取消收藏 +「↗」在 Gmail 中打开（用快照里的 _profileDir 决定 Chrome 资料，空则系统默认浏览器）。纯回看——不提供 AI/模板回复、不参与已读隐藏
...
| 收藏评论 | `FavoriteReviewsPage.vue` | 挂在 Review 工作区 `review-favorites`。评论卡片右下角 ★（`ReviewPage.vue` 里）收藏后，整条评论快照存进 `reviewFavorites.ts`（`review-fav-v1`，scopedKey 隔离，键=review_id），这里按收藏时间倒序列出，卡片只读展示 + 右下角 ★ 点击取消收藏 + （config 齐全时）「在网页中打开」；无 AI 回复/模板回复/分析入口 |
...
| Gmail 页 | `GmailPage.vue` | 读 Apps Script 同步出的 Google Sheet（手动粘贴表链接，存 localStorage `gmail-sources-v1`，每张表可配 Chrome profile + 关联邮件模板产品）：复用 `read_sheet`/`get_sheet_tabs` 读 `Mail` tab、按表头名取列；列表每封固定 3 行（发件人+日期 / 主题 / 机翻中文），「详情」弹大卡（机翻上 / 原文下），「↗」按配的 Chrome profile 打开邮件深链（`open_url_in_chrome_profile`），「已读」本地隐藏（localStorage `gmail-read-ids-v1`，「↩ 撤销上一封」LIFO），「☆/★」收藏该封（存整封快照到 `mail-fav-v1`，详情弹窗里也有一颗）。绕开 Gmail OAuth Testing 7 天过期，全程见 `gmail-handoff.md` |
| 收藏邮件 | `FavoriteMailsPage.vue` | 挂在邮件工作区 `gmail-favorites`。GmailPage 卡片/详情里点 ☆ 后，整封邮件 + 当时的邮件源信息（label/profileDir）存进 `mailFavorites.ts`（`mail-fav-v1`，**全局不 scopedKey**，键=messageId 缺失退回 link），这里按收藏时间倒序列出、按邮件源分 tab，★ 取消收藏 +「↗ 在 Gmail 中打开」（用快照里的 profileDir）。存快照的意义：邮件标已读隐藏、源表被脚本重写、甚至邮件源被删掉，收藏的这封仍能完整回看 |
