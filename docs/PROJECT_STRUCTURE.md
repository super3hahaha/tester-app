...
│   ├── App.vue                  # 根组件：认证状态路由（登录页 ↔ 主页）；watch(user) 把 UserInfo.id 同步写入 activeAccountId（账号隔离维度的唯一来源），并首启触发 migrateLegacyStorageOnce
...
│   │   ├── activeAccount.ts     # 【账号隔离】模块级 `activeAccountId`（后端 UserInfo.id 下发的 opaque 账号标识）+ getActiveAccountId()；唯一写入点在 App.vue 的 watch(user)
...
| 批量回复 · 配置 | `BatchReplyConfigPage.vue` | 多 app 卡片：每张卡片独立**日期预设**（自上一个工作日 / 昨天 / 今天 / 近 7 天 / 自定义）+ 星级；非自定义预设下显示「实际范围」预览（每分钟刷新一次，跨午夜自动重算）；顶部「全选/全不选/刷新/清空配置/保存」工具栏（清空配置 = 删除所有版本 localStorage key 并恢复出厂默认，带 confirm）；显式保存到 localStorage `batch-reply-multi-config-v3`（从旧 key v2/v1 迁移时**强制把日期预设重置为默认**，因为早期 v2 会把"自定义"这个迁移产物写进每个 app；读当前 v3 则保留用户显式选择）；应用列表缓存在 `batch-reply-apps-cache-v1` 让冷启动秒回。**账号隔离**：apps-cache 已经 scopedKey 化（与 Play 配置页同一份缓存）；`batch-reply-multi-config-v3` / `batch-reply-manual-ids-v1` 属隔离二阶段，暂未 scoped |
