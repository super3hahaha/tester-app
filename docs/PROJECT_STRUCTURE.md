...
│       ├── FavoriteReviewsPage.vue # 收藏评论：挂在 Review 工作区（`review-favorites`）。列出 reviewFavorites.ts 里存的收藏评论快照（按收藏时间倒序），卡片复用 ReviewPage 同款展示（星级/正文/原文/meta/开发者回复），右下角 ★ 点击取消收藏；卡片最下方有**备注区**（无备注只显示「＋ 添加备注」，有则黄色便签块 + 编辑/删除，删除走 confirm）；顶部有 favoritesError banner，写失败（损坏保护/配额满）时**不刷新列表、不关编辑态**，用户刚敲的内容不丢；「在网页中打开」只读 `review-page-config-v3`（ReviewPage 维护的 developerId/appIdByPkg）拼深链，缺失则不显示按钮。**「🤖 AI 回复」独立实现**（与 ReviewPage 入口一致但不共用状态，避免跨页面耦合）：`generate_single_reply` 生成 3 条候选 → 选中/编辑 → `reply_to_review` 提交后回填收藏记录（`updateFavoriteReply`）并读-改-写该 app 的评论快照（`reviews-cache/{账号}__{包名}.json`，让 ReviewPage 回到该 app 也能看到最新回复），候选也可「➕ 添加模板」（`product_for_package` + `add_template`）。无「模板回复」「🔍 分析」入口
