...
│       ├── FavoriteMailsPage.vue # 收藏邮件：挂在邮件工作区（`gmail-favorites`）。列出 mailFavorites.ts 里的邮件快照（按收藏时间倒序），按邮件源分 tab，列表卡片与 Gmail 页同款紧凑三行（发件人+日期+📎+收藏时间 / 主题 / 机翻中文，均单行截断），「详情」弹同款大卡（机翻上/原文下），★ 取消收藏 +「↗」在 Gmail 中打开（用快照里的 _profileDir 决定 Chrome 资料，空则系统默认浏览器）；列表卡片最下方与详情弹窗底部各有一处**备注区**（共用同一套编辑状态，改哪边都同步——弹窗持有的是 loadList() 前的旧对象引用，保存后按 key 重新指过去）；顶部有 favoritesError banner，写失败时不刷新列表、不关编辑态/弹窗。纯回看——不提供 AI/模板回复、不参与已读隐藏
...
| 收藏评论 | `FavoriteReviewsPage.vue` | 挂在 Review 工作区 `review-favorites`。评论卡片右下角 ★（`ReviewPage.vue` 里）收藏后，整条评论快照存进 `reviewFavorites.ts`（`review-fav-v1`，scopedKey 隔离，键=review_id），这里按收藏时间倒序列出，卡片只读展示 + 右下角 ★ 点击取消收藏 + 卡片底部备注（增/改/删，存 `note`/`noteUpdatedAt` 字段）+ （config 齐全时）「在网页中打开」；无 AI 回复/模板回复/分析入口 |
