...
- **场景**：批量回复前人工先筛一遍，把**不适合走 AI 模板批量**的评论标出来。这些评论里有的确实不用回复，有的需要回复但模板不合适（用户会手动写或用 AI 单条回复），有的是对 AI 已生成的文案不满意、想单独盯着看再发。
- **语义几经调整**：最初（v1）标记只排除「匹配模板并填充」，不影响一键提交全部——结果用户反馈：对 AI 生成的回复不满意时点「人工处理」，实际诉求是「这条先别跟着一键提交走」，而不是「这条不参与模板匹配」。所以**现在标记同时排除「匹配 / AI 生成回复」和「一键提交全部」**，仍可手动填写 / AI 单条回复 / 逐条点「提交」。之前 v1 版本文档里提到的"曾经踩过反向的坑"（标记误把输入框也禁用、连手填提交都不让）依然作数——教训是"标记别连累手动操作"，跟这次改动（标记该不该拦一键提交）是两件事、不冲突。
- **实现**：`Candidate` 加 `manual` 字段；`buildSkillGroups` 跳过 `manual` 项（不参与 AI 生成），`canBulkSubmit` 也排除 `manual` 项（不参与一键提交）；`totalSubmittable` 依赖 `canBulkSubmit`，输入框 / 逐条提交按钮一律不看 `manual`（手动路径永远可用）。
- **为什么要持久化**：候选每次拉取都重建（走 `syncCandidates` 按 review_id 增量对齐），而被标记的评论在 Play 上仍是未回复态、每次拉取都会重现。不落盘标记就会丢。
...
- **取消标记即恢复**：取消后该评论重新参与模板匹配 + 一键提交，标记从 Set 里删掉并落盘。
...
## Batch Reply 与 Play Console 打通：共享评论数据层 + 共用一份拉取配置

- **背景**：两页原本各自 `list_play_reviews` 拉一次、各存各的内存、各写各的过滤，导致同一条评论在两页是两个不同对象：在 A 页回复了，B 页毫无感知仍列为「未回复」；两页的评论集合也可能对不上（Batch Reply 硬编码「只看未回复」、没有 replyState 维度，配置还是另一份）。用户要求两页展示**同一批评论**、且任一页回复后另一页立刻不再展示这条。
- **去掉 Batch Reply 的独立配置**（`batch-reply-multi-config-v3` + `BatchReplyConfigPage.vue` 一起删除），改用 Play Console 那份 `play-console-multi-config-v1`：两页要展示同一批评论，配置就必须是同一份，留两份必然再次跑偏（删除前两份的 `normalizeConfig` 已经在 `custom` 预设的处理上不一致了）。**replyState 也一起套用**（不是硬编码 ABSENT）：配置选「回复后又更新」时，那些评论确实需要重新回一次，Batch Reply 把它们纳入候选是合理的，也让两页口径完全一致。
- **共享的是内存对象，不只是「已回复 id 黑名单」**：新增 `utils/reviewsStore.ts`，同一账号+包名只有一份响应式数组，两页拿到同一批对象引用；`markReplied()` 原地改 `developer_reply` + 落盘，另一页的过滤 computed 自动重算就把这条剔除了。轻量方案（提交后广播一个 id 让另一页自行过滤）只能同步「已回复」这一个信号，评论集合本身仍是各拉各的、对不齐，解决不了需求的前半截。
- **不需要为「每天累积不遗漏」另做缓存**：Play 的 reviews API 本身只回最近约 7 天的全量评论，两页都是拉全量后本地按日期窗口 + replyState 过滤——只要 replyState 选「无回复」，昨天没回的今天自然还在列表里。所谓「每天缓存累积」是这个口径的自然结果，`reviews-cache/` 快照的作用是免重拉，不承担累积逻辑。**（2026-09-21 修正：这条当时写的「留住 7 天窗口外的旧评论」与代码不符 —— `upsert`/`save_snapshot` 都是覆盖写，超窗口的评论会被抹掉；长假场景下这是真问题，已改成累积，见下面「评论快照改为累积归档」。）**
...
- **选择**：新增 `draftsStore.ts`，草稿落 localStorage（按账号分区，不分包名），`syncCandidates` 建新候选时先查有没有对应 review_id 的持久化草稿、有就回填，不用重新生成。
- **为什么是 localStorage 不是磁盘文件（Tauri command）**：草稿是文本，量级和 `manual-ids`/`favorites` 这些已有的 localStorage 存储相当，没必要新增 Rust command + 前后端联调；且这只是「进程重启兜底」，不是核心数据源，出故障影响可控。
- **口径与评论快照一致——按当次结果整体覆盖，不做实时逐字保存**：`mergeDrafts()` 只在每轮 `generateReplies()` 结束后调用一次，把这一轮结果（含未匹配/异常的）整体合并写入；提交成功后 `removeDraft()` 清掉对应条目，避免堆积、也避免评论「回复后又更新」再次出现时被过期草稿污染。代价：生成后到提交前如果手动改了文案，这层持久化保存的还是「生成时那版」，重启会丢最新的手改内容——可接受，这只是防「一整批重新生成」的兜底，不是逐字段自动保存。
- **与「定时批量生成走前端触发、不在 Rust 重做一遍」（上面那条）不冲突**：那条决策是关于「生成逻辑」该在哪层跑（前端复用 `generateReplies()`，不在 Rust 里重新实现一遍去 spawn claude），这条是关于「生成完的结果要不要多一层持久化」，是生成之后的独立环节，没有引入新的生成路径或并发互斥问题。

## 评论「已读」标记：本地视图状态，不进评论快照

- **动机**：只有「回复」能让评论离开「无回复」视图，那些看过就行、不打算回复的评论（典型是 4★ 好评）会永远占着 Play Console 页和 Batch Reply 页的列表。「已读」给这类评论一个终结状态。
- **为什么不进评论快照（`reviews-cache/*.json`）**：`reviewsStore.upsert()` 每次拉取都用 API 返回的字段原地覆盖，自定义字段活不过一次刷新；后端 `reviews.rs` 和定时线程也写同一份文件，加字段要前后端一起动。所以另起一张 localStorage 表（`review-read-v1`，scopedKey 按账号隔离，键 review_id 跨 app 共用），只在前端过滤时消费。
- **模块级共享响应式，不是每页各存一份**：收藏的 `favIds` 是每个页面各自的 `ref<Set>`（各页只读自己的）；已读要同时驱动 ReviewPage 的 `filtered` 和 BatchReplyPage 的 `visibleByPkg`，所以 `readMarks` 做成模块级 ref —— 任一页标记，另一页的 computed 立刻重算，与 `markReplied` 后两页同步消失是同一种机制。
- **撤销不另建栈**：「上一条」直接取表里 `at` 最大的那条。栈要另外持久化、还要处理与「已读视图里逐条标回未读」的一致性；取 max 天然跨会话有效、可连点连撤，实现和心智都更省。
- **隐藏范围 = 全部筛选（用户定）**：不是只在「无回复」下隐藏。已读表达的是「这条我处理完了」，切到「已回复/回复后又更新」也不该再冒出来；要看就切「只看已读」视图。
- **改评/追评后保持已读（用户定）**：不按 `user_comment_ts` 变新自动复活。自动复活会让「我明明处理过」的评论反复回来，与这个功能要解决的问题相反；真要留意变化的场景由定时 Telegram 通知负责（它只看新增，不读已读表）。
- **30 天自动清理**：Play 评论 API 只回最近 ~7 天，更早的标记对应的评论早就不在任何列表里，留着只让这张表无限膨胀。阈值远大于 7 天，撤销/查看已读不会被误清。
- **写失败不动视图**：`saveMapSafe` 返回 false（损坏保护 / 配额满）时保持评论在列表里，原因走各页已有的 errorMsg/overallError banner —— 与收藏同策略，避免「UI 里消失了但磁盘没记，重启又回来」。

## 评论快照改为累积归档（不再每次覆盖）

- **动机**：用户要放超过 7 天的长假（电脑不关、app 挂着、定时线程每天拉取 + 推 Telegram），回来希望在 app 里看到这 10 天的全部评论。实际做不到：Google 的 reviews API 只返回最近约 7 天，而前端 `upsert()` 会把「本次没返回的」从数组里删掉、后端 `schedule::save_snapshot` 整份覆盖写 —— 假期头几天的评论会被静默抹掉，Telegram 报告又默认只列 5 条摘要（`max_items_in_msg`），等于没有任何地方留下这批数据。
- **合并写放在 `save_reviews_snapshot`（后端）**：前端 `writeSnapshot` 和后端定时线程 `schedule::save_snapshot` 都调这个命令 —— 合并逻辑写在这一层，两条写路径自动都是累积的，不会一条累积一条覆盖。前端 `upsert` 做同口径的内存侧合并（否则页面上要等重启读快照才看得到）。
- **保留 180 天**：覆盖任何假期，又能把文件压在几 MB（每 app 每 7 天约 300 条 ≈ 200KB）。前后端各有一个常量（`RETAIN_DAYS` / `SNAPSHOT_RETAIN_DAYS`），改的时候要一起改，否则会出现「页面上有、重启后没了」。
- **看和回都解决了**：一开始判断「超窗口大概率回不了、只能去 Console 网页」，据此打算做跳转兜底；用户当场在收藏评论页实测推翻了这个判断（2026-09-21）——**7 天窗口只卡 reviews.list，不卡 reviews.reply**，手上有 reviewId 就能回。所以累积下来的旧评论在 app 里照常 AI 回复 / 模板回复 / 一键提交，**兜底功能不做**，超窗口评论在 UI 上与普通评论完全一致（不加标记、不拦截提交）。详见 gotchas.md。
- **日期下限跟着放宽**：`minSelectableDate` 原本锁死 7 天前，累积后必须放宽到本地最早那条评论，否则攒下来的评论永远筛不出来；另加一个「本地全部（自 YYYY-MM-DD）」预设按钮，长假回来一键拉到最早。
- **`markReplied` 改合并写（500ms debounce）**：快照涨到几 MB 后，「一键提交全部」逐条整份读-改-写会明显拖慢提交。代价是最后一次写落盘前进程被杀会丢几条本地回复状态 —— 无害，回复在 Play 侧已生效，下次拉取带回来。

## 收藏标签：邮件 → app 靠邮件源上的直接字段，不走模板链（2026-09-24）

- 邮件本身没有 app 信息。可选路径：邮件源 → 邮件模板产品 → GP 模板产品 → 包名。**没采用**：两套模板库（`templates/` 与 `email-templates/`）产品名只是大小写不同的「约定」，改个名就断；邮件源可以不配模板；「纯生成」app（product=null，如 Video Player）根本没有产品可对。
- 采用：邮件源加 `appPkg`，模板链只用来**首次自动推断**（undefined 才推），之后以用户手选为准。
- 收藏邮件不快照 appPkg，实时按 `_sourceKey` 查源 —— 用户改关联后老收藏立即跟着变。
