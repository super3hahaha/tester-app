...
- **不需要为「每天累积不遗漏」另做缓存**：Play 的 reviews API 本身只回最近约 7 天的全量评论，两页都是拉全量后本地按日期窗口 + replyState 过滤——只要 replyState 选「无回复」，昨天没回的今天自然还在列表里。所谓「每天缓存累积」是这个口径的自然结果，`reviews-cache/` 快照的作用是免重拉、留住 7 天窗口外的旧评论，不承担累积逻辑。
