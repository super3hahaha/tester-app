...
## Gmail 同步不能用「未读」当队列游标（会永久丢邮件）

**现象**：用户对我方回复的追问，在 Gmail 里能看到，但表里和 app 里永远没有。

**根因**：Gmail 的「标记为已读」是按**整条会话**生效的，而且网页端点开会话就会自动标已读。旧版 `syncOneLabel_` 用 `if (!msg.isUnread()) continue` 筛待同步邮件，所以只要一封新邮件在脚本跑之前被人看过一眼（回复完顺手标已读、或纯粹点开瞄了下），它就被永久跳过——脚本每天只跑一次，人点邮件是随时的。

**实测证据**（2026-08-07，`gmail-audit-missing.gs` 对账 + 同一次 `syncMail()` 的天然对照）：

| 邮件 | 同步前状态 | 结果 |
|---|---|---|
| FADIL（用户追问，被手动点开过） | 已读 | **没进表，且再也不会进** |
| Kirill（用户追问，没点开） | 未读 | 正常入表 |

同一次运行、同一个标签，唯一变量就是已读状态。

**修法**：判重游标改用「表里有没有这个 messageId」（`getExistingMessageIds_` 本来就有），已读状态降级为纯视觉提示。

**改的时候必须一并处理的两件事**（否则引入新 bug）：
1. **显式排除我方发出的邮件**（`isFromMe_`）。原来靠「自己发的天然是已读」顺带过滤掉了，不看已读之后，我方回复会被当成用户来信写进表。
2. **加时间窗口剪枝**（`SYNC_WINDOW_DAYS`）。老邮件原本被已读状态挡着，换判重方式后会被一次性翻出来重灌——尤其是 `gmail-clear-backlog.gs` 清到独立表格的那批历史积压。用 `thread.getLastMessageDate()` 在 `getMessages()` 之前剪枝，消息级再用 `internalDate` 卡一道。

**顺带澄清一个误解**：旧代码的「只扫未读」并不省时间。`isUnread()` 是在 `thread.getMessages()` **之后**在内存里判的，扫描范围完全由 `MAX_THREADS` 决定，和已读状态无关（真正靠未读缩小服务器侧范围的写法是 `GmailApp.search("... is:unread")`，见 `gmail-clear-backlog.gs`）。实测 200 个线程全量遍历约 24 秒，50 个线程约 6 秒，离 Apps Script 6 分钟上限很远；耗时大头一直是逐封 `translateZh_` 机翻。

**往 HEADERS 末尾加列时**：`ensureHeader_` 旧实现只看第一格是不是 `messageId` 就 return，老表永远补不上新列头，app 侧 `colIndex` 会一直返回 -1。已改成逐列比对。
